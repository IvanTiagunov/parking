...
from app.schemas.user import UserSchemaFull, UserCreateData, DriverCreateData, MechanicCreateData
...
@router.post("/driver/create", response_model=MechanicCreateData)
async def create_mechanic(user: get_us, mechanic_data: MechanicCreateData, session: Session = Depends(get_db)):
    ...
    create_mechanic_crud(session, user, mechanic_data)
    return user
